...
export class ReviewTool {
  ...
  private async validateFiles(files: string[]): Promise<void> {
    ...
    if (invalidFiles.length > 0) {
      throw new ValidationError(`Invalid files: ${invalidFiles.join(', ')}`);
    }
  }

  private async performReview(args: ReviewToolArgs, context: ToolContext): Promise<ReviewResult> {
    ...
    const overallRecommendations = await this.generateRecommendations(findings, args);
  ...
  private async reviewFile(
    ...
    try {
      ...
      for (const query of queries) {
        const result = await this.commandExecutor.execute('gh', ['copilot', 'suggest', query], {
  ...
  private buildReviewQueries(file: string, _content: string, args: ReviewToolArgs): string[] {
    ...
    // Security
    if (
      args.includeSecurity &&
      (args.focusAreas.length === 0 || args.focusAreas.includes('security'))
    ) {
      queries.push(
    ...
    if (
      ...
      (args.focusAreas.length === 0 || args.focusAreas.includes('performance'))
    ) {
      queries.push(
