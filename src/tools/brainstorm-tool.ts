...
import { BrainstormResult, BrainstormIdea } from '../types/index.js';
...
export class BrainstormTool {
  ...
  private async generateIdeas(
    ...
    const sortedIdeas = scoredIdeas.sort(
  ...
  private formatBrainstormResult(result: BrainstormResult): string {
    ...
    if (result.ideas.length > 0) {
      ...
      for (let i = 0; i < result.ideas.length; i++) {
        ...
        lines.push('');
        lines.push(
          `   📈 Feasibility: ${idea.feasibility}/10 | Impact: ${idea.impact}/10 | Complexity: ${idea.complexity}/10`
        );
